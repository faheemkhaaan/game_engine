...
export class BoidSystem {
    ...
    update() {
        ...
        for (const entity of entites) {
            ...
            if (seperation) seperation.scale(boidComponent.seperationWeight);
            if (cohision) cohision.scale(boidComponent.cohesionWeight);
            if (alignment) alignment.scale(boidComponent.alignmentWeight);
     ...
     * @returns {Vector}
     */
    seperation(entity: Entity, entities: Entity[]) {
        /*
  ...
  */
    wallAvoidance(entity: Entity) {
        ...
        let closestWall: Entity | null = null;
