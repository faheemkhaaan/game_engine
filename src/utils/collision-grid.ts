...
export class CollisionGrid {
    ...
    private staticEntities = new Map<string, Set<Entity>>()
    private dynamicEntities = new Map<string, Set<Entity>>()

    constructor(private cellSize = 200) {
    ...
    addStaticEntity(entity: Entity) {
        ...
        for (let x = minX; x <= maxX; x += this.cellSize) {
            for (let y = minY; y <= maxY; y += this.cellSize) {
                ...
                const entitySet = this.staticEntities.get(key);
                if (entitySet) entitySet.add(entity)
            }
        }
    }
    /**
    ...
    */
    updateDynamicEntity(entity: Entity) {
        ...
        for (let x = startX; x <= endX; x += this.cellSize) {
            for (let y = startY; y <= endY; y += this.cellSize) {
                ...
                const entitySet = this.dynamicEntities.get(key);
                if (entitySet) entitySet.add(entity)
            }
        }
    }
    ...
    getPotentialBoidsOnly(dynamicEntity: Entity): Entity[] {
        const pos = dynamicEntity.transform.pos;
        const candidates = new Set<Entity>();
        ...
        return Array.from(candidates); // return Set directly, avoid Array.from
    }
    getNearbyWalls(dynamicEntity: Entity) {
        const pos = dynamicEntity.transform.pos;
        const candidates = new Set();
     ...
     */
    static aabbOverlap(pos1: Vector, width1: number, height1: number, pos2: Vector, width2: number, height2: number) {
